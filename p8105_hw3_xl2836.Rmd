...
There are `r asile_num` aisles and the "dairy eggs" aisle is the most items ordered from. Also, we can find that those which most items ordered from are sneaks. Besides, the number of aisle which most items ordered from and the number of aisle which least items ordered from are significant different.
...
According to the table, we can find 0 is the most commonly observed values.

### Question 2

```{r}
ny_noaa_clean %>% 
  filter(month == "01" | month == "05") %>% 
  group_by(year, id) %>% 
  summarise(aver_tem = mean(tmax, na.rm = TRUE))
```
...
```{r}
ny_noaa_clean %>% 
  filter(sonw > 0 & snow <100) %>% 
  ggplot(aes(x = year, fill = snow)
```
